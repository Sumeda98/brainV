      ...
      <div className="flex items-center px-5 lg:px-7.5 xl:px-10 max-lg:py-4">
        ...
        <nav className='hidden fixed top-[5rem] left-0 right-0 bottom-0 bg-n-8 lg:static lg:flex lg:mx-auto lg:bg-transparent'>
            <div className='relative flex flex-col items-center justify-center m-auto z-2 lg:flex-row'>
                {navigation.map((item) =>(
                    <a key={item.id} href={item.url} className={`block relative font-code text-2xl uppercase text-n-1 transition-colors hover:text-color-1 ${item.onlyMobile? 'lg:hidden' : ''} px-6 py-6 md:py-8 lg:mr-0.25 lg:text-sm lg:font-semibold ${item.url === pathname.hash ? "z-2 lg:text-n-1":"lg:text-n-1/50"} lg:leading-5 lg:hover:text-n-1 xl:px-12`} >
                        {item.title}
                    </a>
                ))}
            </div>
        </nav>
        <a href='#signup' className='hidden mr-8 transition-colors button text-n-1/50 hover:text-n-1 lg:block'>New account</a>
